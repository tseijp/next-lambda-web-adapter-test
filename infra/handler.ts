...
import { all, fileArgs, one, run } from "./utils";
...
const DB_PATH = isLocal ? "./db.sqlite3" : "/mnt/db/db.sqlite";
...
export const routes = app
  ...
  // blobs create
  .post("/blobs", json({ file: z.custom<File>() }), async (c) => {
    const q = /* SQL */ `INSERT INTO blobs (file_data, file_name, file_size, file_type) VALUES (?, ?, ?, ?)`;
    const { file } = c.req.valid("json");
    const args = await fileArgs(file);
    const ok = await run(q, ...args);
    return c.json({ ok });
  })
  // blobs update by id
  .patch(
    "/blobs/:id",
    json({ id: z.string(), file: z.custom<File>() }),
    async (c) => {
      const q = /* SQL */ `UPDATE blobs SET file_data = ?, file_name = ?, file_size = ?, file_type = ?, updated_at = datetime("now") WHERE id = ?`;
      const { file } = c.req.valid("json");
      const args = await fileArgs(file);
      const id = c.req.param("id");
      const ok = await run(q, ...args, id);
      return c.json({ ok });
    }
  )
  // blobs delete by id
  ...
  .get("/items/forms/:id", async (c) => {
    const q = /* SQL */ `SELECT * FROM items WHERE form_id = ?`;
    const id = c.req.param("id");
    const res = all<Items>(q, id);
    return c.json(res);
  })
  .post(
    "/items",
    ...
    async (c) => {
      const q = /* SQL */ `INSERT INTO items (pathname, blob_id, form_id, content) VALUES (?, ?, ?, ?)`;
      const { pathname, blob_id, form_id, content } = c.req.valid("json");
      const ok = await run(q, pathname, blob_id, form_id, content);
      return c.json({ ok });
    }
  )
  .patch(
    ...
    async (c) => {
      ...
      const id = c.req.param("id");
      const { pathname, blob_id, form_id, content } = c.req.valid("json");
      const ok = await run(q, pathname, blob_id, form_id, content, id);
      return c.json({ ok });
    }
  )
